...
local function palette(c,p)
    if gbMode == 0 then
        ...
        sb.logWarn("Attempting to render in CGB mode!")
        -- TODO
    end
end
...
-- I use M-cycles here
function init()
    drawable = root.assetJson("/scripts/quarterscreen.json").drawable
    local drawables = {}
    ...
    message.setHandler("doFrame",function(_,_,writes)
        ...
        for scanline=0,vblank-1 do
            ...
            if objEnabled then
                for i=1,40 do
                    local v = objects[i]
                    if v[2] > 0 and scanline >= v[1]-16 and scanline < v[1]+objSize-16 then
        ...
        --[[
        sb.setLogMap("gbppu_r_mode",string.format("%d",mode))
        ...
        sb.setLogMap("gb_renderTime",string.format("%.4f",os.clock()-time))
        return drawables
    end)
